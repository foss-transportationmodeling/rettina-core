from mapmatching.gps_point_cand_point.GPS_point import ObsPoint
from mapmatching.hmm_procedure.transition_weight import TransitionWeight
import pickle as pkl
import numpy as np
import pandas as pd
from hmm_procedure.read_file import read_data_point
#import time
#import sys
import os
...
def Viterbi(datafile, lon_col_id, lat_col_id, timestamp_col_id,
            ...
            road_net_multigraph_pickled=None,
            beta=1):
    '''
...
if __name__ == '__main__':
    ...
    road_net_shp = os.path.join(data_folder,
                                'LineString_Road_Network_UTM.shp')
    datafile = os.path.join(data_folder, r'phnGPS_orng.csv')
    road_net_multigraph_pickled = os.path.join(data_folder,
                                               r'MultiGraph.p')
    out = Viterbi(datafile=datafile,
        lon_col_id=5, lat_col_id=4, timestamp_col_id=9,
        gps_mean = 0, gps_std_dev=7, circ_radius=30,
        road_net_shp = road_net_shp,
        road_net_multigraph_pickled = road_net_multigraph_pickled,
        beta=1)
    print out[2], "\n", out[3]
